...
import { Search, ChevronRight, PlusCircle, Inbox } from 'lucide-react';
...
import { getTramitesBySolicitante } from '../services/firebase';
...
export default function MisTramitesScreen() {
  ...
  useEffect(() => {
    getTramitesBySolicitante(user.uid)
      .then(setTramites)
      .catch(console.error)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800 }}>Mis Trámites</h2>
          <p style={{ margin: '0.2rem 0 0', color: 'var(--text-muted)', fontSize: '0.8rem' }}>{filtered.length} solicitudes tuyas</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
        ...
        <select value={filtroEstatus} onChange={e => setFiltroEstatus(e.target.value as any)} style={{ padding: '0.6rem 0.75rem', border: '1px solid var(--border-soft)', borderRadius: '0.65rem', fontSize: '0.875rem' }}>
          {allStatuses.map(s => <option key={s} value={s}>{s === 'TODOS' ? 'Todos' : ESTATUS_LABELS[s as Estatus]}</option>)}
        </select>
      </div>

      {filtered.length === 0 ? (
        <div style={{ background: 'white', borderRadius: '1rem', padding: '3rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid var(--border-soft)', textAlign: 'center' }}>
          <Inbox size={48} color="var(--brand-300)" style={{ margin: '0 auto 1rem', display: 'block' }} />
          <h3 style={{ margin: '0 0 0.5rem', fontSize: '1rem', fontWeight: 700, color: 'var(--text-muted)' }}>Sin trámites</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', margin: '0 0 1.25rem' }}>Aún no has realizado ninguna solicitud</p>
          <button onClick={() => navigate('/nuevo-tramite')} className="btn-institutional">Crear mi primera solicitud</button>
        </div>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead>
              <tr style={{ background: 'var(--surface-soft)' }}>
                {['Folio', 'Tipo', 'Fecha', 'Estado', 'Vence', ''].map(h => (
                ...
                return (
                  <tr key={t.id} style={{ borderBottom: '1px solid var(--border-soft)', cursor: 'pointer' }} onClick={() => navigate(`/tramites/${t.id}`)}>
                    ...
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <span style={{ fontSize: '0.72rem', fontWeight: 700, background: COLOR_ESTATUS[t.estatus] + '22', color: COLOR_ESTATUS[t.estatus], padding: '0.2rem 0.55rem', borderRadius: '999px', whiteSpace: 'nowrap' }}>{ESTATUS_LABELS[t.estatus]}</span>
                    </td>
                    <td style={{ padding: '0.75rem 1rem' }}>
                      {dLeft !== null ? <span style={{ fontSize: '0.75rem', fontWeight: 700, color: colorPorDiasRestantes(dLeft) }}>{estaVencido(t.fechaLimiteEntrega!) ? `${Math.abs(dLeft)}d venc` : `${dLeft}d`}</span> : <span style={{ color: 'var(--text-muted)' }}>—</span>}
                    </td>

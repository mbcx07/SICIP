...
export default function LoginScreen() {
  ...
  // Guardar usuario en sessionStorage para ProtectedRoute
  useEffect(() => {
    const unsubscribe = onAuthChange(async (firebaseUser) => {
      if (firebaseUser) {
        // Login por matrícula NO usa Firebase Auth — verificar via sessionStorage
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { loginWithMatricula } = await import('../services/firebase');
      const usuario: Usuario = await loginWithMatricula(matricula, password);
      sessionStorage.setItem('sicip_usuario', JSON.stringify(usuario));
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem', marginTop: '0.5rem' }}>
          <div style={{ width: 72, height: 72, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}>
          ...
          <h1 style={{ fontSize: '2.25rem', fontWeight: 900, letterSpacing: '0.08em', margin: '0 0 0.25rem 0', background: 'linear-gradient(135deg, #1a5c32, #27ae60)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            SICIP
          </h1>
          ...
          <div>
            ...
            <input
              ...
              style={{
                ...
                transition: 'border-color 0.15s', background: '#fafafa', fontFamily: 'inherit',
              }}
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <button type="button" onClick={() => setShowPw(!showPw)}
                style={{ position: 'absolute', right: '1rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: '0.8rem', fontWeight: 600 }}>
                {showPw ? 'Ocultar' : 'Ver'}
              </button>
            </div>
            <p style={{ margin: '0.4rem 0 0 0', fontSize: '0.72rem', color: '#888' }}>
              Primera vez: usa tu matrícula como contraseña
            </p>
          </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '2rem', paddingTop: '1.25rem', borderTop: '1px solid #f0f0f0' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', color: '#adb5bd', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.05em' }}>

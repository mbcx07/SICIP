...
import type { Tramite, HistorialEstado } from '../types/sicip';
...
export default function TramiteDetalleScreen() {
  ...
  useEffect(() => {
    onAuthChange(async (fu) => { if (fu) { const u = await getUsuario(fu.uid); setUser(u); } });
  }, []);
  ...
  if (!tramite) return (
    <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
      ...
      <button onClick={() => navigate(-1)} style={{ color: 'var(--brand-600)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Regresar</button>
    </div>
  );
    ...
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <button onClick={() => navigate(-1)} style={{ background: 'var(--surface-soft)', border: 'none', borderRadius: '0.6rem', padding: '0.5rem', cursor: 'pointer', color: 'var(--text-muted)' }}>
            <ArrowLeft size={18} />
          </button>
          <div>
      ...
      {/* Info Card */}
      <div style={{ background: 'white', borderRadius: '1rem', padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid var(--border-soft)', marginBottom: '1rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem', fontSize: '0.875rem' }}>
          <div><div style={{ fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, marginBottom: '0.15rem' }}>Trabajador</div>{tramite.trabajadorNombre}</div>
          <div><div style={{ fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, marginBottom: '0.15rem' }}>Matrícula</div>{tramite.trabajadorMatricula}</div>
          <div><div style={{ fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, marginBottom: '0.15rem' }}>Unidad</div>{tramite.unidadClave} — {tramite.unidadNombre}</div>
          <div><div style={{ fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, marginBottom: '0.15rem' }}>Fecha creación</div>{formatFechaTime(tramite.fechaCreacion)}</div>
          {tramite.fechaIncidencia && <div><div style={{ fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, marginBottom: '0.15rem' }}>Fecha incidencia</div>{formatFecha(tramite.fechaIncidencia)}</div>}
          {tramite.fechaLimiteEntrega && <div><div style={{ fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, marginBottom: '0.15rem' }}>Fecha límite</div>{formatFecha(tramite.fechaLimiteEntrega)}</div>}
        </div>
        {((tramite.datos as any)?.motivo) && (
          <div style={{ marginTop: '0.75rem', padding: '0.75rem', background: 'var(--surface-soft)', borderRadius: '0.5rem', fontSize: '0.875rem' }}>
            <div style={{ fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, marginBottom: '0.15rem' }}>Motivo</div>
            {(tramite.datos as any).motivo}
          </div>
        )}
      </div>
      ...
      {getNextActions().length > 0 && (
        <div style={{ background: 'white', borderRadius: '1rem', padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid var(--border-soft)', marginBottom: '1rem' }}>
          <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, marginBottom: '0.5rem', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Observaciones</label>
          <textarea value={obs} onChange={e => setObs(e.target.value)} rows={2} placeholder="Agrega una observación (opcional)..."
            style={{ width: '100%', padding: '0.6rem 0.75rem', border: '1px solid var(--border-soft)', borderRadius: '0.6rem', fontSize: '0.875rem', resize: 'vertical', boxSizing: 'border-box', marginBottom: '0.75rem' }} />
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            {getNextActions().map(a => (
              <button key={a.key} onClick={() => handleAction(ACTION_MAP[a.key])} disabled={saving}
                style={{ padding: '0.6rem 1rem', background: a.color, color: 'white', border: 'none', borderRadius: '0.6rem', fontSize: '0.85rem', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.6 : 1, transition: 'opacity 0.15s' }}>
                {a.label}
              </button>
            ))}
          </div>
      ...
      <div style={{ background: 'white', borderRadius: '1rem', padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid var(--border-soft)' }}>
        <h3 style={{ margin: '0 0 1rem', fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)' }}>Historial</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          {historial.length === 0 && <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Sin cambios registrados</p>}
        </div>
      </div>
    </div>
  );
